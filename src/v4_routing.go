...
func handleModelRouteV4(raw []byte) ([]byte, error) {
	...
	fp := fingerprint(key)
	model := stringAny(req, "RequestedModel")
	if model == "" {
		model = stringAny(req, "requested_model")
	}
	...
	if p.ClientAffinity != clientAffinityOff && p.ClientAffinity != clientAffinityStrict {
		observeV4(RoutingEvent{TraceID: traceIDV4(), At: nowV4(), Decision: decisionHandled, Reason: "unsupported_client_affinity", PolicyName: p.Name, KeyFingerprint: fp, KeyHint: p.KeyHint, RuleID: rule.ID, RuleName: rule.Name, Strategy: rule.Strategy, Model: model, Success: false, Error: "unsupported Client Affinity mode: " + p.ClientAffinity}, callbackID)
		return okEnvelope(map[string]any{"Handled": true, "TargetKind": "self", "Target": pluginID, "Reason": "kcr_unsupported_client_affinity"})
	}
...
func handleExecuteV4(raw []byte, streaming bool) ([]byte, error) {
	...
	source := stringAny(req, "SourceFormat")
	if source == "" {
		source = stringAny(req, "source_format")
	}
	if source == "" {
		source = "openai"
	}
	...
	if key == "" {
		return errorEnvelope("policy_key_missing", "无法识别 CPA 原生 API Key"), nil
	}
	fp := fingerprint(key)
	if len(original) > 0 {
		var m map[string]any
		if json.Unmarshal(original, &m) == nil {
			if x := stringAny(m, "model"); x != "" {
				model = x
			}
		}
	}
	p, rule := findPolicyRuleV4(fp, model)
	if p == nil || rule == nil || rule.Strategy == strategyCPADefault {
		return errorEnvelope("policy_not_found", "KCR executor 未找到已匹配 Policy/Rule"), nil
	}
	trace := traceIDV4()
	started := time.Now()
	...
	if streaming {
		...
		if obsV4().ResponseHeaders {
			outHeaders.Set("X-KCR-Decision", decisionHandled)
			outHeaders.Set("X-KCR-Trace-ID", trace)
		}
		return okEnvelope(map[string]any{"headers": outHeaders})
	}
	resp, event, err := runNonStreamPolicyV4(trace, p, rule, ranked, source, model, body, headers, query, alt, callbackID, started)
	...
	if obsV4().ResponseHeaders {
		...
		resp.Headers.Set("X-KCR-Decision", event.Decision)
		resp.Headers.Set("X-KCR-Trace-ID", trace)
		resp.Headers.Set("X-KCR-Rule", rule.Name)
	}
...
func findPolicyRuleV4(fp, model string) (*Policy, *PolicyRule) {
	...
	for i, r := range p.Rules {
		...
		score := ruleMatchScoreV4(r, model)
		if score < 0 {
			continue
		}
		score = score*1000 + (len(p.Rules) - i)
		if score > bestScore {
			bestScore = score
			best = r
		}
...
func ruleMatchScoreV4(r *PolicyRule, model string) int {
	...
	for _, p := range r.Models {
		...
		if wildcardMatch(p, model) {
			literal := len(strings.ReplaceAll(strings.ReplaceAll(p, "*", ""), "?", ""))
			s := 1000 + literal
			if s > best {
				best = s
			}
...
func rankCandidatesV4(p *Policy, r *PolicyRule, headers http.Header, meta map[string]any) []*PolicyCandidate {
	...
	key := p.KeyFingerprint + ":" + r.ID
	switch r.Strategy {
	case strategyRoundRobin:
		v4Runtime.Lock()
		n := v4Runtime.rr[key]
		v4Runtime.rr[key] = n + 1
		v4Runtime.Unlock()
		start := int(n % uint64(len(cs)))
		return append(append([]*PolicyCandidate{}, cs[start:]...), cs[:start]...)
	case strategyWeightedRR:
		first := smoothPickV4(key, cs)
		return weightedRemainderV4(first, cs)
	case strategyPriorityWeighted:
		...
		first := smoothPickV4(key+":"+strconv.Itoa(top), group)
		return priorityRemainderV4(first, cs)
	case strategySticky:
		sticky := stickyKeyV4(r, headers, meta)
		if sticky == "" {
			sticky = p.KeyFingerprint
		}
		sort.SliceStable(cs, func(i, j int) bool {
			if cs[i].Priority != cs[j].Priority {
				return cs[i].Priority > cs[j].Priority
			}
			return rendezvousScoreV4(sticky, cs[i]) > rendezvousScoreV4(sticky, cs[j])
		})
		return cs
	default:
		return cs
...
func smoothPickV4(key string, cs []*PolicyCandidate) *PolicyCandidate {
	...
	cur := v4Runtime.smooth[key]
	if cur == nil {
		cur = map[string]int{}
		v4Runtime.smooth[key] = cur
	}
	total := 0
	var best *PolicyCandidate
	bestVal := math.MinInt
	for _, c := range cs {
		w := c.Weight
		if w <= 0 {
...
func stickyKeyV4(r *PolicyRule, h http.Header, m map[string]any) string {
	source := strings.ToLower(strings.TrimSpace(r.StickySource))
	if source == "" {
		source = "auto"
	}
	if source == "header" && r.StickyHeader != "" {
		return h.Get(r.StickyHeader)
	}
	if source == "session" || source == "auto" {
		for _, k := range []string{"session_id", "sessionId", "trace_id"} {
			if s := stringAny(m, k); s != "" {
				return s
			}
		}
		for _, k := range []string{"X-Claude-Code-Session-Id", "X-Session-Id", "Session-Id", "X-Request-Id"} {
			if s := h.Get(k); s != "" {
				return s
			}
		}
...
func rendezvousScoreV4(key string, c *PolicyCandidate) float64 {
	...
	x := h.Sum64()
	u := (float64(x) + 1) / (float64(math.MaxUint64) + 1)
	w := c.Weight
	if w <= 0 {
		w = 1
	}
	return math.Pow(u, 1/float64(w))
}
